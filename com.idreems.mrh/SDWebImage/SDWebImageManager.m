...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegate>)delegate retryFailed:(BOOL)retryFailed
{
    ...
    NSDictionary *info = [NSDictionary dictionaryWithObjectsAndKeys:delegate, @"delegate", url, kURL, nil];
...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(NSData *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didFinishWithImage:(NSData *)image
{
    [downloader retain];
    ...
    for (NSInteger idx = [downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            {
                [delegate performSelector:@selector(webImageManager:didFinishWithImage:) withObject:downloader.url.absoluteString withObject:image];
            }
